...
cd /mnt/SDCARD/Apps/AnyxCC

# 1. Local version check
LOCAL_VER=$(cat version.txt 2>/dev/null || echo "0.0")

# 2. Download (raw) version.txt from github
REMOTE_VER=$(wget -qO- https://raw.githubusercontent.com/altuux/TSP-CC/main/version.txt 2>/dev/null | tr -d '\n\r')
...
wget -q --show-progress https://github.com/altuux/TSP-CC/archive/refs/heads/main.zip -O update.zip
